...
class Emeasure(object):
    ...
    def cal_em_with_threshold(self, pred: np.ndarray, gt: np.ndarray, threshold: float) -> float:
        ...
        if self.gt_fg_numel == 0:
            ...
            parts_numel, combinations = self.generate_parts_numel_combinations(fg_fg_numel=fg_fg_numel, fg_bg_numel=fg_bg_numel, pred_fg_numel=fg___numel, pred_bg_numel=bg___numel)

            results_parts = []
            for i, (part_numel, combination) in enumerate(zip(parts_numel, combinations)):
                align_matrix_value = 2 * (combination[0] * combination[1]) / (combination[0] ** 2 + combination[1] ** 2 + _EPS)
    ...
    def cal_em_with_cumsumhistogram(self, pred: np.ndarray, gt: np.ndarray) -> np.ndarray:
        ...
        if self.gt_fg_numel == 0:
            ...
            parts_numel_w_thrs, combinations = self.generate_parts_numel_combinations(fg_fg_numel=fg_fg_numel_w_thrs, fg_bg_numel=fg_bg_numel_w_thrs, pred_fg_numel=fg___numel_w_thrs, pred_bg_numel=bg___numel_w_thrs)
...
# Weighted F-measure类，用于计算加权F-measure指标
